...
class EDecoder implements ObjectInput {
    ...
    private void processFirstMsg() throws IOException {
        ...
        if (m_serverVersion == REDIRECT_MSG_ID) {
            ...
            if (m_clientMsgSink != null) {m_clientMsgSink.redirect(newAddress);}
        ...
        if (m_serverVersion >= 20) {
            ...
            if (m_clientMsgSink != null) {m_clientMsgSink.serverVersion(m_serverVersion, twsTime);}
        } else {
            if (m_clientMsgSink != null) {m_clientMsgSink.serverVersion(m_serverVersion, null);}
        }
    ...
    private void processExecutionDataMsg() throws IOException {
        ...
        if (m_serverVersion >= EClient.MIN_SERVER_VER_FRACTIONAL_POSITIONS) {exec.shares(readDouble());} else {
            exec.shares(readInt());
        }
